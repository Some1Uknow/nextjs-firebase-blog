...
export default function HomePage() {
  ...
  const postAllUsersToFirestoreDatabase = async () => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(
        query(usersCollection, where("userEmail", "==", user.email))
      );
  ...
  return (
    <main className="select-none min-h-screen w-full p-3">
      <DropdownMenu>
        <DropdownMenuTrigger className=" rounded-xl border-2 border-gray-600 px-4 py-2">
          My Profile
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          ...
          <div className="block">
            <div className="flex items-center justify-center h-full w-full">
              <div className="mr-24 h-[430px] w-[450px] space-y-3 border-2 border-gray-800 rounded-xl px-10 py-6">
                ...
                <Input
                  placeholder="Give a mindblowing title 🤯"
                  value={articleTitle}
                ...
                <div className="flex justify-between">
                  ...
                  <Button onClick={discardPost} variant="destructive">
                    Discard
                  </Button>
                </div>
              </div>
      ...
      <div className="mt-10">
        <h1 className="text-center text-gray-300 text-2xl">Recent feeds 👇</h1>

        <div className="grid grid-cols-1 space-y-4 mt-5">
          {allArticles.map((article) => (
            <div
              key={article.articleTitle}
              className="h-auto w-auto p-2 border-2 border-gray-800 rounded-xl overflow-hidden"
            >
